...
private:
    ...
    std::span<data::pixel_colour_t> image_span_;
    char load[1024]{};

    template <std::size_t pixel_colour_buffer_size,
              std::size_t filter_buffer_size>
    friend class render_buffer;
    data::pixel_colour_t *write_colours_to_buffer(data::pixel_colour_t *);

    template<std::size_t SIZE>
    std::size_t write_filters_to_buffer(std::array<filters::filter_types, SIZE>& filter_buffer, std::size_t free_index);  //std::span<filters::filter_types>, std::size_t);
};
